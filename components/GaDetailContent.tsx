          ...
          {mode === 'choose' && (
            <div className="space-y-3">
              <div className="flex gap-3 bg-gray-50 rounded-xl p-3">
                {ga.ga_images?.[0] ? (
                  <img src={ga.ga_images.find((i: any) => i.is_primary)?.url || ga.ga_images[0].url}
                    alt={ga.ten} className="w-14 h-14 object-cover rounded-xl flex-shrink-0" />
                ) : (
              ...
              <button onClick={() => setMode('qua-san')}
                className="w-full text-left border-2 border-green-500 rounded-xl p-4 hover:bg-green-50 transition">
              ...
              <button onClick={() => setMode('tu-mua')}
                className="w-full text-left border-2 border-orange-300 rounded-xl p-4 hover:bg-orange-50 transition">
                <div className="flex items-start gap-3">
          ...
          {mode === 'qua-san' && (
            <>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-4">
                <div className="font-bold text-blue-800 mb-3 text-sm">📱 Quét QR đặt cọc</div>
              ...
              {nguoiBan?.phone && nguoiBan?.phone_visibility !== 'private' ? (
                <div className="space-y-2 mb-4">
                  <a href={`tel:${nguoiBan.phone}`}
                    className="flex items-center gap-3 bg-green-50 border border-green-200 text-green-800 font-bold py-3 px-4 rounded-xl hover:bg-green-100 transition">
                  ...
                  <a href={`https://zalo.me/${nguoiBan.phone}`} target="_blank"
                    className="flex items-center gap-3 bg-blue-50 border border-blue-200 text-blue-800 font-bold py-3 px-4 rounded-xl hover:bg-blue-100 transition">
                    <span className="text-xl">💬</span>
                    <div><div className="text-xs font-normal text-blue-600">Zalo</div><div>{nguoiBan.phone}</div></div>
                  </a>
                </div>
              ) : (
                <div className="bg-gray-50 rounded-xl p-4 text-center text-gray-400 text-sm mb-4">
      ...
      <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-4 space-y-3">
          <div className="flex items-center gap-3 bg-gray-50 rounded-xl p-3">
            <div className="w-11 h-11 bg-[#8B1A1A] rounded-full flex items-center justify-center text-white font-black">
...
// ─────────────────────────────────────────────
// GA DETAIL CONTENT — component dùng chung
// Props:
//   gaId     : string  — ID của gà cần hiển thị
//   isModal  : boolean — true = trong modal (ẩn breadcrumb, giới hạn scroll bình luận)
//   onClose  : khi là modal, nút đóng gọi hàm này
// ─────────────────────────────────────────────
...
export default function GaDetailContent({ gaId, isModal = false, onClose }: GaDetailContentProps) {
  ...
  const [editForm, setEditForm] = useState({ ten: '', gia: '', mo_ta: '' });
  const [gaState, setGaState] = useState<any>(null); // local state để cập nhật sau khi sửa

  const isOwner = !!(currentUser && ga && String(currentUser.id) === String(ga.user_id));
  const gaData = gaState || ga; // dùng gaState nếu đã update, fallback về ga
  ...
  async function fetchGa() {
    ...
    try {
      ...
      if (gaRaw) {
        setGa(gaRaw);
        setGaState(gaRaw);
        setEditForm({ ten: gaRaw.ten, gia: String(gaRaw.gia), mo_ta: gaRaw.mo_ta || '' });
        ...
        const { data: cmts } = await supabase.from('comments')
          .select('*, profiles(username)').eq('ga_id', gaId)
          .order('created_at', { ascending: false });
        setComments(cmts || []);

        // Tăng view count (chỉ khi không phải modal để tránh tăng 2 lần)
        if (!isModal) {
          await supabase.from('ga').update({ view_count: (gaRaw.view_count || 0) + 1 }).eq('id', gaId);
        }
      }
  ...
  async function addComment() {
    ...
    const { data } = await supabase.from('comments')
      .insert({ ga_id: gaId, user_id: currentUser.id, noi_dung: comment })
  ...
  async function handleTraGia() {
    ...
    if (convId) {
      await openChat({ convId, type: 'product', gaId: gaData.id, gaTen: gaData.ten, gaAnh: anhGa, doiPhuongId: gaData.user_id });
    }
  ...
  async function handleSaveEdit() {
    ...
    setGaState((prev: any) => ({ ...prev, ten: editForm.ten, gia: editForm.gia, mo_ta: editForm.mo_ta }));
    setActionLoading(false);
    setShowEdit(false);
  }
  ...
  return (
    <div className={wrapperClass}>
      ...
      {/* Breadcrumb — chỉ hiện khi không phải modal */}
      {!isModal && (
      ...
      {/* Status badge (trong modal hiện ở đây, ngoài trang không cần vì có breadcrumb) */}
      {isModal && (
      ...
      <div className="grid md:grid-cols-2 gap-5 mb-5">
            ...
            <>
              <div className="relative bg-black rounded-xl overflow-hidden mb-2" style={{ paddingBottom: '65%' }}>
                {currentMedia.type === 'image' ? (
                  <img src={currentMedia.url} alt={gaData.ten}
                    className="absolute inset-0 w-full h-full object-contain" />
                ) : currentYtId ? (
                  !isPlaying ? (
                    <>
                      <img src={currentMedia.thumb || `https://img.youtube.com/vi/${currentYtId}/hqdefault.jpg`}
                    ...
                    <iframe src={`https://www.youtube.com/embed/${currentYtId}?autoplay=1&controls=1&rel=0`}
        ...
        <div className="flex flex-col gap-3">
          {/* Tên + loại (chỉ ngoài trang) */}
          {!isModal && (
            <>
              <div className="text-xs text-[#8B1A1A] font-semibold">{gaData.loai_ga}</div>
              <h1 className="font-black text-2xl text-gray-800 leading-tight">{gaData.ten}</h1>
            </>
          )}
          {isModal && (
            <h2 className="font-black text-xl text-gray-800 leading-tight">{gaData.ten}</h2>
          )}
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {!isModal && gaData.view_count > 0 && (
              <span className="bg-gray-100 text-gray-700 px-3 py-1 rounded-full text-sm">👁 {gaData.view_count} lượt xem</span>
            )}
          </div>

          {/* Nút hành động (chỉ cho người khác, không phải chủ) */}
          {!isOwner && (
            <div className="flex gap-2 flex-wrap">
              <button onClick={() => {
                if (!currentUser) { alert('Vui lòng đăng nhập!'); return; }
                if (currentUser.id === gaData.user_id) { alert('Đây là gà của bạn!'); return; }
                setShowMuaNgay(true);
              }} className="flex-1 bg-[#8B1A1A] text-white font-black py-3 rounded-xl hover:bg-[#6B0F0F] transition min-w-[100px] text-sm">
                🛒 Mua ngay
              </button>
              <button onClick={handleTraGia} disabled={chatLoading}
                className="flex-1 border-2 border-[#8B1A1A] text-[#8B1A1A] font-bold py-3 rounded-xl hover:bg-red-50 transition min-w-[100px] text-sm disabled:opacity-60">
                {chatLoading ? '⏳...' : '💬 Trả giá'}
              </button>
              <button onClick={() => setShowLienHe(true)}
                className="border-2 border-gray-300 text-gray-600 font-bold px-4 py-3 rounded-xl hover:bg-gray-50 transition text-sm">
                📞
              </button>
            </div>
          )}
          ...
          {aiData ? (
                  ...
                  {[
                    ...
                    <div key={item.label} className="bg-white rounded-lg p-2">
                      ...
                      <div className="h-1.5 bg-gray-200 rounded-full mt-1">
                        <div className="h-1.5 bg-[#8B1A1A] rounded-full" style={{ width: getBarWidth(item.score) }} />
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-3 text-center text-gray-400 text-sm">
      ...
      <div className={`grid gap-5 ${isModal ? 'md:grid-cols-1' : 'md:grid-cols-3'}`}>
        ...
        {/* Cột trái (2/3) — Mô tả + Chủ gà (quản lý) + Bình luận */}
              ...
              {showEdit ? (
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setShowEdit(false)} className="flex-1 border border-gray-300 text-gray-600 py-2.5 rounded-xl text-sm font-bold hover:bg-gray-50">Hủy</button>
                  ...
                  {gaData.status === 'active' && (<>
                    <div className="flex gap-2">
                      <button onClick={() => setShowEdit(true)}
                        className="flex-1 bg-blue-50 text-blue-700 text-xs font-bold py-2 rounded-lg hover:bg-blue-100">✏️ Sửa thông tin</button>
                      <button onClick={() => updateStatus('hidden')} disabled={actionLoading}
                        className="flex-1 bg-white border text-gray-600 text-xs font-bold py-2 rounded-lg hover:bg-gray-50">👁️ Ẩn bài</button>
                    </div>
                    <button onClick={() => updateStatus('in_transaction')} disabled={actionLoading}
                      className="w-full bg-yellow-50 text-yellow-700 text-xs font-bold py-2 rounded-lg hover:bg-yellow-100">🤝 Đánh dấu đang giao dịch</button>
                    <button onClick={handleDelete} disabled={actionLoading}
                      className="w-full bg-red-50 text-red-600 text-xs font-bold py-2 rounded-lg hover:bg-red-100">🗑️ Xóa bài đăng</button>
                  </>)}
                  {gaData.status === 'hidden' && (<>
                    <button onClick={() => updateStatus('active')} disabled={actionLoading}
                      className="w-full bg-green-50 text-green-700 text-xs font-bold py-2 rounded-lg hover:bg-green-100">🟢 Hiện lại bài đăng</button>
                    <button onClick={handleDelete} disabled={actionLoading}
                      className="w-full bg-red-50 text-red-600 text-xs font-bold py-2 rounded-lg hover:bg-red-100">🗑️ Xóa</button>
                  </>)}
                  {gaData.status === 'in_transaction' && (<>
                    <button onClick={() => updateStatus('pending_completion')} disabled={actionLoading}
                      className="w-full bg-blue-50 text-blue-700 text-xs font-bold py-2 rounded-lg hover:bg-blue-100">✅ Xác nhận đã giao gà</button>
                    <button onClick={() => updateStatus('active')} disabled={actionLoading}
                      className="w-full bg-white border text-gray-600 text-xs font-bold py-2 rounded-lg hover:bg-gray-50">↩️ Huỷ giao dịch</button>
                  </>)}
                  {gaData.status === 'pending_completion' && (
                    <button onClick={() => updateStatus('pending_dispute')} disabled={actionLoading}
                      className="w-full bg-orange-50 text-orange-700 text-xs font-bold py-2 rounded-lg hover:bg-orange-100">⚠️ Hoàn tất + mở khiếu nại 3 ngày</button>
                  )}
                  {gaData.status === 'pending_dispute' && (
                    <button onClick={() => updateStatus('sold')} disabled={actionLoading}
                      className="w-full bg-purple-50 text-purple-700 text-xs font-bold py-2 rounded-lg hover:bg-purple-100">✅ Đóng khiếu nại — Hoàn tất bán</button>
                  )}
                  {gaData.status === 'sold' && (
                    <button onClick={handleDelete} disabled={actionLoading}
                      className="w-full bg-red-50 text-red-600 text-xs font-bold py-2 rounded-lg hover:bg-red-100">🗑️ Xóa</button>
        ...
        {/* Cột phải (1/3) — Người bán + Gà tương tự (chỉ ngoài trang) */}
        {!isModal && (
          <div className="space-y-4">
            <div className="bg-white rounded-xl p-4 shadow-sm">
              ...
              <div className="flex gap-2">
                <Link href={`/ho-so/${gaData.user_id}`}
        ...
        {/* Người bán — hiện trong modal ở phần dưới */}
        {isModal && nguoiBan && (
          <div className="flex items-center justify-between bg-gray-50 rounded-xl p-3">
            <div className="flex items-center gap-2">
              <div className="w-9 h-9 bg-[#8B1A1A] rounded-full flex items-center justify-center text-white font-black text-sm">
                {(nguoiBan.username || 'U')[0].toUpperCase()}
              </div>
              <div>
                <div className="font-bold text-sm text-gray-800">{nguoiBan.username || 'Người bán'}</div>
                <div className="text-xs text-gray-500">⭐ {nguoiBan.trust_score || 5.0} uy tín</div>
              </div>
            </div>
            <Link href={`/ho-so/${gaData.user_id}`} onClick={onClose}
