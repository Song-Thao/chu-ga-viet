...
export default function HeaderChatSection({ user }: { user: any }) {
  ...
  function setupRealtime(userId: string) {
    supabase.channel(`hdr-notif-${userId}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, async (payload) => {
        ...
        const doiPhuongId = (conv as any).buyer_id === userId ? (conv as any).seller_id : (conv as any).buyer_id;
        setNotifications(prev => [{
          id: msg.id, conv_id: conv.id,
          type: (conv as any).type || 'product',
          ga_id: conv.ga_id, ga_ten: gaData?.ten || 'Gà', ga_anh: gaAnh,
          doi_phuong_id: doiPhuongId,
          noi_dung: msg.noi_dung, time: new Date().toISOString(), read: false,
        }, ...prev.slice(0, 19)]);
  ...
  async function handleNotifClick(n: any) {
    ...
    await openChat({
      convId: n.conv_id, type: n.type || 'product',
      gaId: n.ga_id, gaTen: n.ga_ten, gaAnh: n.ga_anh,
    ...
    if (next && user) {
      ...
      if (convData) {
        const enriched = await Promise.all(convData.map(async (c: any) => {
          ...
          const gaAnh = (ga as any)?.ga_images?.find((i: any) => i.is_primary)?.url || (ga as any)?.ga_images?.[0]?.url || '';
          return { ...c, ga_ten: (ga as any)?.ten || 'Gà', ga_anh: gaAnh, doi_phuong_id: dpId, doi_phuong_name: dp?.username || 'Người dùng', last_msg: lastMsg };
        }));
  ...
  async function handleConvClick(c: any) {
    setShowInbox(false);
    await openChat({
      convId: c.id, type: c.type || 'product',
      gaId: c.ga_id, gaTen: c.ga_ten, gaAnh: c.ga_anh,
      doiPhuongId: c.doi_phuong_id, doiPhuongName: c.doi_phuong_name,
    });
  }
  ...
  return (
    <>
      ...
      <div className="relative" ref={notifRef}>
        <button onClick={() => { setShowNotif(n => !n); setShowInbox(false); if (!showNotif) setUnreadNotif(0); }}
          className="relative hover:text-yellow-300 transition p-1">
                ...
                {conversations.map(c => (
                  <button key={c.id} onClick={() => handleConvClick(c)}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {c.last_msg && <div className="text-xs text-gray-400 truncate">{c.last_msg.noi_dung}</div>}
                    </div>
                    {c.last_msg && <div className="text-xs text-gray-400 flex-shrink-0">{timeAgo(c.last_msg.created_at)}</div>}
                  </button>
                ))}
              </div>
            )}
