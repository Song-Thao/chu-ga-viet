'use client';
// app/cho/ChoClient.tsx — chỉ xử lý filter/sort/modal ở client
// Data ban đầu đã được server fetch sẵn → không cần chờ JS
      ...
      <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[95vh] overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-3 border-b sticky top-0 bg-white z-10">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={onClose} className="w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center hover:bg-gray-200 text-gray-600 text-sm">✕</button>
          </div>
        </div>
      ...
      <div className="fixed inset-0 bg-black/65 z-50 flex items-center justify-center p-3" onClick={onClose}>
        <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
          <div className="flex items-center justify-between px-3 py-2.5 border-b">
            ...
            <button onClick={onClose} className="flex-shrink-0 w-7 h-7 bg-gray-100 rounded-full flex items-center justify-center hover:bg-gray-200 text-gray-600 text-xs">✕</button>
          </div>

          {mediaList.length > 0 && (
            <div className="relative bg-black" style={{ paddingBottom: '56%' }}>
              {currentMedia.type === 'image' ? (
                <Image src={currentMedia.url} alt={ga.ten} fill className="object-contain" sizes="400px" />
              ) : ytId ? (
                !isPlaying ? (
                  <>
              ...
              {aiScore && (
                <div className="absolute top-2 right-2 bg-yellow-400 text-black text-xs px-2 py-0.5 rounded-full font-bold z-10">⭐ {aiScore}</div>
              )}
            </div>
          )}
            ...
            <div className="flex gap-1.5 px-3 pt-2 overflow-x-auto">
              {mediaList.map((m, i) => (
                <button key={i} onClick={() => setActiveMedia(i)}
                  ...
                  <img src={m.type === 'image' ? m.url : m.thumb || ''} alt="" className="w-full h-full object-cover" loading="lazy" />
                  {m.type === 'video' && (
...
// ─────────────────────────────────────────────
// CHO CLIENT — nhận initialData từ server
// ─────────────────────────────────────────────
export default function ChoClient({ initialData }: { initialData: any[] }) {
  ...
  // Filter + sort hoàn toàn client-side từ data đã có
