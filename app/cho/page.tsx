'use client';
// app/cho/page.tsx — giữ 'use client', fix bằng cách cache API đúng cách
      ...
      <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[95vh] overflow-y-auto shadow-2xl" onClick={e=>e.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-3 border-b sticky top-0 bg-white z-10">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={onClose} className="w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center text-sm">✕</button>
          </div>
        </div>
...
function GaCard({ ga, idx, priority=false }: { ga: any; idx: number; priority?: boolean }) {
  ...
  return (
    <>
      ...
      <div onClick={()=>setShowModal(true)} className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition cursor-pointer">
        <div className="relative h-36 w-full bg-gray-100">
          {displayThumb ? (
            <Image src={displayThumb} alt={ga.ten} fill sizes="(max-width:640px) 50vw,(max-width:1024px) 33vw,25vw" className="object-cover" priority={priority} loading={priority?'eager':'lazy'} />
          ) : (
  ...
  return (
    <div className="bg-white rounded-xl p-4 shadow-sm">
      ...
      <div className="mb-4">
        ...
        <input type="range" min="500000" max="50000000" step="500000" value={giaMax} onChange={e=>setGiaMax(Number(e.target.value))} className="w-full accent-red-800" aria-label="Mức giá" />
      </div>
      <div className="mb-4">
        <div className="text-sm font-semibold text-gray-600 mb-2">Khu vực</div>
        <select value={khuVuc} onChange={e=>setKhuVuc(e.target.value)} className="w-full border rounded-lg px-2 py-1.5 text-sm" aria-label="Khu vực">
        ...
        <div className="space-y-1 max-h-48 overflow-y-auto">
          {LoaiGa.map(l=>(
            <label key={l} className="flex items-center gap-2 text-sm cursor-pointer hover:text-red-800">
              <input type="radio" name="loaiga" checked={loaiGa===l} onChange={()=>setLoaiGa(l)} className="accent-red-800" />{l}
            </label>
          ))}
        </div>
      </div>
...
export default function ChoPage() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
        // next: { revalidate: 60 } → Vercel cache 60s
        const res = await fetch('/api/ga?limit=24', {
          next: { revalidate: 60 },
        } as any);
        ...
        <div className="flex-1 min-w-0">
          <div className="flex justify-between items-center mb-3">
            <div>
              ...
              <p className="text-xs text-gray-600">{loading ? '...' : `${sorted.length} kết quả`}</p>
            </div>
          ...
          {loading ? <SkeletonGrid /> : sorted.length > 0 ? (
            <>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {sorted.map((ga,idx)=><GaCard key={ga.id} ga={ga} idx={idx} priority={idx<4} />)}
              </div>
