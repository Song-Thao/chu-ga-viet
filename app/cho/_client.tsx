...
function GaQuickModal({ ga, onClose }: { ga: any; onClose: () => void }) {
  ...
  const mediaList = [
    ...(ga.ga_images||[]).map((img: any) => ({ type: 'image', url: img.url })),
          ...
          {mediaList.length > 0 && (
            <div className="relative bg-black" style={{ paddingBottom: '56%' }}>
              {cur.type === 'image' ? (
                ...
                <>
                  <img src={(cur as any).thumb||`https://img.youtube.com/vi/${ytId}/hqdefault.jpg`} alt="video" className="absolute inset-0 w-full h-full object-cover" loading="lazy" />
            ...
            <div className="flex gap-1.5 px-3 pt-2 overflow-x-auto">
              {mediaList.map((m, i) => (
                <button key={i} onClick={() => setActiveMedia(i)} className={`relative flex-shrink-0 w-10 h-10 rounded-lg overflow-hidden border-2 ${activeMedia===i?'border-[#8B1A1A]':'border-gray-200'}`}>
                  <img src={m.type==='image'?m.url:(m as any).thumb||''} alt="" className="w-full h-full object-cover" loading="lazy" />
...
export default function ChoPageClient({ initialData }: { initialData: any[] }) {
